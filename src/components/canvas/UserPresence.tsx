  ...
  return (
    <div className="flex items-center gap-1">
      {visibleUsers.map((user, index) => (
        ...
        >
          <div
            className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium border-2 shadow-soft cursor-pointer"
            style={{ 
          ...
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-3 py-1.5 bg-foreground text-background text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 shadow-elevated pointer-events-none">
            ...
            <div className="text-background/70 text-[10px] uppercase tracking-wider">
              {getRoleBadge(user.role)}
            </div>
            {user.id === currentUserId && (
              <span className="text-[10px] text-background/50">(You)</span>
            )}
          </div>
        </motion.div>
      ))}
